...
This fork is adapted to work with gcc/g++ compilers instead of clang. It has been tested on Ubuntu 14.04 and 16.04.

## Note 
If you compile this project with gcc/g++, you won't be able to use it as a plugin for Unreal. The plugin has to be compiled using Clang using the original repo. 

The aim of this fork is to be able to link the client side of AirSim against your project that requires the gcc compiler because of dependencies (e.g. Linux binaries like OpenCV, ROS etc) that cannot be linked using Clang. 

Hence, the Unreal Engine plugin should be compiled using the original repo (for server side), while you should link your gcc/g++ project with the shared libraries generated by this fork (for client side APIs). An example usage (a ROS project) can be found in [https://github.com/rakeshshrestha31/publishAirsimImgs](https://github.com/rakeshshrestha31/publishAirsimImgs). 
